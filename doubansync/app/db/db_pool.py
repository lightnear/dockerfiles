...
class DBPool(object):
    ...
    def __init__(self, max_active=5, max_wait=10, init_size=0, db_type="SQLite3", **config):
        ...
        for i in range(init_size):
            self.logger.debug("[DB] 初始化数据库连接%s" % str(i))
        ...
        try:
            ...
            if self.__free_conns.empty():  # 如果容器是空的，直接创建一个连接
                ...
                try:
                    ...
                    # to do
                    self.logger.warning("[DB] 获取连接失败: %s" % str(err))
            return conn
        finally:
            self.__lock.release()
    ...
    def free(self, conn):
        ...
        try:
            ...
            if self.__free_conns.full():  # 如果当前连接池已满，直接关闭连接
                conn.close()
                self.logger.debug("[DB] 关闭连接")
                return
            try:
                self.logger.debug("[DB] 回收连接")
                self.__free_conns.put_nowait(conn)
            except Exception as err:
                self.logger.error("[DB] 当前线程池已满，无法放回, 直接释放！%s" % str(err))
                conn.close()
        finally:
            self.__lock.release()
